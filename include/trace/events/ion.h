...
DECLARE_EVENT_CLASS(oplus_bp_alloc,

	TP_PROTO(size_t len,
		 int prio,
		 char *dbg_name),

	TP_ARGS(len, prio, dbg_name),
	...
	TP_printk("len=%zu prio=%d dbg_name=%s",
		__entry->len,
		__entry->prio,
		__entry->dbg_name)
);

DEFINE_EVENT(oplus_bp_alloc, bp_alloc_start,
...
DEFINE_EVENT(oplus_bp_alloc, bp_alloc_end,
