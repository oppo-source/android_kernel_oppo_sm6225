...
static unsigned long damos_wmark_metric_value(enum damos_wmark_metric metric)
{
	...
	case DAMOS_WMARK_AVAI_MEM_RATE:
		si_meminfo(&i);
		return si_mem_available() * 1000 / i.totalram;
	default:
		break;
	}
	return -EINVAL;
...
static unsigned long damos_wmark_wait_us(struct damos *scheme)
{
	...
	if (metric > scheme->wmarks.high || scheme->wmarks.low > metric) {
		if (scheme->wmarks.activated)
			printk("deactivate a scheme (%d) for %s wmark\n",
					scheme->action,
...
static int kdamond_fn(void *data)
{
	...
	while (!kdamond_need_stop(ctx) && !done) {
		...
		//kdamond_usleep(ctx->sample_interval > ctx->punish_interval ?
		//	ctx->sample_interval : ctx->punish_interval);
