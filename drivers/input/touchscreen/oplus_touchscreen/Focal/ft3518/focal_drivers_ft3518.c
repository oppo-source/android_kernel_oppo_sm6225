...
/**
 * get_hw_id1_status -using for detecting with hwid1
 * return hw_flag	-no pull, pull-up, pull-down
 */
int get_hw_id1_status(int gpio_name)
{
	int ret, active_val, sleep_val, hw_flag = 0;

	ret = pinctrl_select_state(fts_data->hw_res->pinctrl, fts_data->hw_res->pin_hw1_high);
	if (ret < 0) {
		TPD_INFO("Set hwid1 high pin state error:%d", ret);
	}
	active_val = gpio_get_value(gpio_name);

	ret = pinctrl_select_state(fts_data->hw_res->pinctrl, fts_data->hw_res->pin_hw1_low);
	if (ret < 0) {
		TPD_INFO("Set hwid1 low pin state error:%d", ret);
	}
	...
	gpio_free(gpio_name);
	fts_data->hw_res->pin_hw1_high = NULL;
	fts_data->hw_res->pin_hw1_low = NULL;
	return hw_flag;
}

int get_hw_id2_status(int gpio_name)
{
	int ret, active_val, sleep_val, hw_flag = 0;

	ret = pinctrl_select_state(fts_data->hw_res->pinctrl, fts_data->hw_res->pin_hw2_high);
	if (ret < 0) {
		TPD_INFO("Set hwid2 active pin state error:%d", ret);
	}
	active_val = gpio_get_value(gpio_name);

	ret = pinctrl_select_state(fts_data->hw_res->pinctrl, fts_data->hw_res->pin_hw2_low);
	if (ret < 0) {
		TPD_INFO("Set hwid2 sleep pin state error:%d", ret);
	}
	sleep_val = gpio_get_value(gpio_name);
	/**
	 * hw_flag = 0  high res 0
	...
	gpio_free(gpio_name);
	fts_data->hw_res->pin_hw2_high = NULL;
	fts_data->hw_res->pin_hw2_low = NULL;
	return hw_flag;
}

int ts_get_hw_id_status(struct fts_ts_data *fts_data)
{
	int ret1 = 0, ret2 = 0;
	int changed_pcb_flag = 0;
	ret1 = get_hw_id1_status(fts_data->hw_res->hw_id1_gpio);
	ret2 = get_hw_id2_status(fts_data->hw_res->hw_id2_gpio);

	TPD_INFO("%s, ret1 = %d, ret2 = %d", __func__, ret1, ret2);
	if ((ret1 == 1 && ret2 == 1) || (ret1 == 1 && ret2 == 2)) {
		changed_pcb_flag = 0;	/*use gpio 86*/
	} else {
		changed_pcb_flag = 1;	/*use gpio 100*/
	}
	return changed_pcb_flag;
}
EXPORT_SYMBOL(ts_get_hw_id_status);
...
int fts_pinctrl_select_active(struct fts_ts_data *fts_data)
{
	...
	if (fts_data->hw_res->pinctrl && fts_data->hw_res->pin_set_high) {
		ret = pinctrl_select_state(fts_data->hw_res->pinctrl, fts_data->hw_res->pin_set_high);
		TPD_INFO("%s : ret = %d\n", __func__, ret);
		if (ret < 0) {
...
int fts_pinctrl_select_suspend(struct fts_ts_data *fts_data)
{
	...
	if (fts_data->hw_res->pinctrl && fts_data->hw_res->pin_set_low) {
		ret = pinctrl_select_state(fts_data->hw_res->pinctrl , fts_data->hw_res->pin_set_low);
		TPD_INFO("%s : ret = %d\n", __func__, ret);
		if (ret < 0) {
...
static int fts_rstgpio_set(struct hw_resource *hw_res, bool on)
{
	if (fts_data->reset_pinctrl) {	/*limit use pinctrl or normal to operate gpio*/
		...
		if (changed_pcb_flag == 0) {	/*limit use hwid to compatibilite diff pcb*/
			TPD_INFO("use gpio 86 for rst");
			fts_data->hw_res->pin_set_high = fts_data->hw_res->pin_rst2_high;
			fts_data->hw_res->pin_set_low = fts_data->hw_res->pin_rst2_low;
		}
		TPD_INFO("%s: changed_pcb_flag = %d\n", __func__, changed_pcb_flag);
		if (on == true) {
...
static int ft3518_parse_dts(struct fts_ts_data *ts_data, struct i2c_client *client)
{
	...
	if (ts_data->reset_pinctrl) {
		ts_data->reset_hwid = of_property_read_bool(np, "reset-hwid");
		TPD_INFO("%s: use hwid to limit diff gpio = %d\n", __func__, ts_data->reset_hwid);

		ts_data->hw_res->hw_id1_gpio = of_get_named_gpio(np, "hw-id1-gpio", 0);
		TPD_INFO("%s: get hwid1 gpio = %d\n", __func__, ts_data->hw_res->hw_id1_gpio);

		ts_data->hw_res->hw_id2_gpio = of_get_named_gpio(np, "hw-id2-gpio", 0);
		TPD_INFO("%s: get hwid2 gpio = %d\n", __func__, ts_data->hw_res->hw_id2_gpio);
		...
		if (IS_ERR_OR_NULL(ts_data->hw_res->pinctrl)) {
			...
			ts_data->hw_res->pin_rst2_high = pinctrl_lookup_state(ts_data->hw_res->pinctrl, "pin_rst2_high");
			if (IS_ERR_OR_NULL(ts_data->hw_res->pin_rst2_high)) {
				TPD_INFO("Failed to get rst2 high state pinctrl handle\n");
				ret = PTR_ERR(ts_data->hw_res->pin_rst2_high);
				goto err_pinctrl_lookup;
			}

			ts_data->hw_res->pin_rst2_low = pinctrl_lookup_state(ts_data->hw_res->pinctrl, "pin_rst2_low");
			if (IS_ERR_OR_NULL(ts_data->hw_res->pin_rst2_low)) {
				TPD_INFO(" Failed to get rst2 low state pinctrl handle\n");
				ret = PTR_ERR(ts_data->hw_res->pin_rst2_low);
				goto err_pinctrl_lookup;
			}

			ts_data->hw_res->pin_hw1_high = pinctrl_lookup_state(ts_data->hw_res->pinctrl, "pin_hw1_high");
			if (IS_ERR_OR_NULL(ts_data->hw_res->pin_hw1_high)) {
				TPD_INFO("Failed to get hwid1 high state pinctrl handle\n");
				ret = PTR_ERR(ts_data->hw_res->pin_hw1_high);
				goto err_pinctrl_lookup;
			}

			ts_data->hw_res->pin_hw1_low = pinctrl_lookup_state(ts_data->hw_res->pinctrl, "pin_hw1_low");
			if (IS_ERR_OR_NULL(ts_data->hw_res->pin_hw1_low)) {
				TPD_INFO(" Failed to get hwid1 low state pinctrl handle\n");
				ret = PTR_ERR(ts_data->hw_res->pin_hw1_low);
				goto err_pinctrl_lookup;
			}

			ts_data->hw_res->pin_hw2_high = pinctrl_lookup_state(ts_data->hw_res->pinctrl, "pin_hw2_high");
			if (IS_ERR_OR_NULL(ts_data->hw_res->pin_hw2_high)) {
				TPD_INFO("Failed to get hwid2 high state pinctrl handle\n");
				ret = PTR_ERR(ts_data->hw_res->pin_hw2_high);
				goto err_pinctrl_lookup;
			}

			ts_data->hw_res->pin_hw2_low = pinctrl_lookup_state(ts_data->hw_res->pinctrl, "pin_hw2_low");
			if (IS_ERR_OR_NULL(ts_data->hw_res->pin_hw2_low)) {
				TPD_INFO(" Failed to get hwid2 low state pinctrl handle\n");
				ret = PTR_ERR(ts_data->hw_res->pin_hw2_low);
				goto err_pinctrl_lookup;
			}
		}
		changed_pcb_flag = ts_get_hw_id_status(ts_data);
		TPD_INFO("%s : changed_pcb_flag = %d\n", __func__, changed_pcb_flag);
	}
	return 0;
err_pinctrl_lookup:
...
err_pinctrl_get:
	...
	ts_data->hw_res->pin_hw2_low = NULL;
	ts_data->hw_res->pin_hw2_high = NULL;
	ts_data->hw_res->pin_hw1_low = NULL;
	ts_data->hw_res->pin_hw1_high = NULL;
	ts_data->hw_res->pin_rst2_low = NULL;
	ts_data->hw_res->pin_rst2_high = NULL;
