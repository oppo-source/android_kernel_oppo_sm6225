...
/*add BCT4482 support*/
#define BCT4482_DEVICE_REG_VALUE 0x09
						...
						struct fsa4480_priv *fsa_priv)
{
	...
	case TYPEC_ACCESSORY_AUDIO:
		...
		#ifdef OPLUS_ARCH_EXTENDS
		/* Add DIO4480 support */
		if((fsa_priv->vendor != DIO4480) && (fsa_priv->vendor != DIO4483) && (fsa_priv->vendor != BCT4482)) {
			/* Add for open auto mic DET */
...
static int typec_switch_to_fast_charger(struct fsa4480_priv *fsa_priv, unsigned long to_fast_charger)
{
	...
	if ((fsa_priv->vendor != WAS4783) && (fsa_priv->vendor != DIO4483) && (fsa_priv->vendor != BCT4482)
	...
	struct device *dev)
{
	...
	pr_info("%s: fsa_priv->usb_protocal = %d\n", __func__, fsa_priv->usb_protocal);
			 ...
			 const struct i2c_device_id *id)
{
	...
	} else if (reg_value == HL5281_DEVICE_REG_VALUE) {
	...
	} else if (reg_value == BCT4482_DEVICE_REG_VALUE) {
		dev_info(fsa_priv->dev, "%s: switch chip is BCT4482\n", __func__);
	...
	if (rc || use_powersupply == 0) {
		...
		if (fsa_priv->usb_protocal != 1) {
			...
			if (rc) {
				dev_err(fsa_priv->dev, "%s: tcpc notifier registration failed: %d\n",
					__func__, rc);
				goto err_data;
			}
#endif
...
static int fsa4480_remove(struct i2c_client *i2c)
{
	...
	if ((fsa_priv->vendor == DIO4483) || (fsa_priv->vendor == BCT4482)) {
		unregister_chg_glink_notifier(&fsa_priv->chg_nb);
	}
	/*add HL5281 support*/
	if (fsa_priv->vendor == HL5281) {
