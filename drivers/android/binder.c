				    ...
				    struct binder_thread *thread)
{
	...
	if (thread) {
		...
		trace_android_vh_binder_special_task(t, proc, thread, &t->work, &proc->todo, !oneway, &enqueue_task);
		if (enqueue_task)
		...
		trace_android_vh_binder_special_task(t, proc, thread, &t->work, &node->async_todo, !oneway, &enqueue_task);
  		if (enqueue_task)
		...
		struct binder_buffer *buffer, bool is_failure)
{
	...
	if (buffer->async_transaction && buffer->target_node) {
		...
		if (!w) {
			...
			trace_android_vh_binder_special_task(NULL, proc, thread, w,  &proc->todo, false, &enqueue_task);
  			if (enqueue_task)
