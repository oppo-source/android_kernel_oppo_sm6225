...
static bool (*poplus_gauge_chip_is_null)(void);
...
void oplus_get_gauge_chip_is_null_pfunc(bool (*pfunc)(void))
{
	poplus_gauge_chip_is_null = pfunc;
}
EXPORT_SYMBOL(oplus_get_gauge_chip_is_null_pfunc);
...
static void oplus_i2c_gpio_reset_work(struct work_struct *work)
{
	int ret = 0;
	int i = 0;
	int boot_mode = MSM_BOOT_MODE__NORMAL;//get_boot_mode();
...
{
	bool ret = false;
	if (poplus_gauge_chip_is_null == NULL) {
		dev_err(gi2c->dev, "poplus_gauge_chip_is_null is null, skip %s\n", __func__);
		return ret;
	}

	if (poplus_gauge_chip_is_null()) {
		dev_err(gi2c->dev, "gauge chip is null, skip %s\n", __func__);
		return ret;
	}
...
/* If other devices access the i2c process of fuel gauge reset, the I2C reset will not be successful.
 * At the same time, it will cause abnormal communication waveforms.
 */
#ifdef OPLUS_FEATURE_CHG_BASIC
	if (gi2c->i2c_reset_processing) {
		I2C_LOG_ERR(gi2c->ipcl, true, gi2c->dev,
...
#ifdef OPLUS_FEATURE_CHG_BASIC
		for (i = 0; i < num; i++) {
			if (msgs[i].addr == FG_DEVICE_ADDR || msgs[i].addr == FG_CW2217_ADDR) {
