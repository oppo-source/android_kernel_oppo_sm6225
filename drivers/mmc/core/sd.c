...
{
    struct mmc_sd_lock *sdlock = NULL;
...
retry:
	...
	if (rocr & SD_ROCR_S18A)
		host->sdlock->swith_voltage = true;
	else
		host->sdlock->swith_voltage = false;
	/*
...
#ifdef CONFIG_MMC_PASSWORDS
	if (status & R1_CARD_IS_LOCKED) {
		if (host->sdlock->auto_unlock) {
			if (host->sdlock->unlock_pwd[0] > 0) {
				/* unlock sd card */
				err = mmc_lock_unlock_by_buf(card,  host->sdlock->unlock_pwd+1, (int)host->sdlock->unlock_pwd[0], MMC_LOCK_MODE_UNLOCK);
				if (err) {
					printk("[SDLOCK] %s unlock failed \n", __func__);
				}
				else {
					if (!mmc_card_locked(card)) {
						host->sdlock->auto_unlock = false;
