...
//#include "../../regulator/oplus_camera_wl2868c.h"
...
#ifdef CONFIG_OPLUS_MISC_HEALTHONFO
...
static int aw2023_2v8_power_on(struct aw2023_led *led, bool on)
{
	int rc = 0;
	...
	if (!led->gpio_pwr_ctl) {
		...
		if (led->enable2v8_gpio > 0) {
			pr_err("aw2023[led]:Enable the 2v8_gpio, hw_res->enable2v8_gpio is %d\n", led->enable2v8_gpio);
			rc = gpio_direction_output(led->enable2v8_gpio, 1);
			if (rc) {
				pr_err("aw2023[led]:enable the enable2v8_gpio failed.\n");
				return rc;
			}
		}
	}
	...
	if (!led->gpio_pwr_ctl) {
		...
		if (led->enable2v8_gpio > 0) {
			pr_err("aw2023[led]:Disable the 2v8_gpio, hw_res->enable2v8_gpio is %d\n", led->enable2v8_gpio);
			rc = gpio_direction_output(led->enable2v8_gpio, 0);
			if (rc) {
				pr_err("aw2023[led]:Disable the enable2v8_gpio failed.\n");
				return rc;
			}
		}
	}
		led->power2v8on = false;
	}
	return rc;
}
...
reg_vcc_put:
	regulator_put(led->vcc);
reg_vdd_set_vtg:
	if ((!led->gpio_pwr_ctl) && regulator_count_voltages(led->vdd) > 0)
		regulator_set_voltage(led->vdd, 0, AW2023_VDD_MAX_UV);
reg_vdd_put:
        if(!led->gpio_pwr_ctl) {
	    regulator_put(led->vdd);
...
{
	u8 i=0, val=0, enable = 0, state = AW2023_LED_OFF;
	...
	case AW2023_LED_RED:
		if (led->cdev.brightness > 0)
			...
			led->pdata->led->rgb_isnk_on &= ~R_ISNK_ON_MASK;
		break;
	case AW2023_LED_GREEN:
		if (led->cdev.brightness > 0)
			led->pdata->led->rgb_isnk_on |= G_ISNK_ON_MASK;
		else
			led->pdata->led->rgb_isnk_on &= ~G_ISNK_ON_MASK;
		break;
	case AW2023_LED_BLUE:
		if (led->cdev.brightness > 0)
	...
	dev_err(&led->pdata->led->client->dev, "rgb_isnk_on = 0x%x\n", led->pdata->led->rgb_isnk_on);
	if (led->cdev.brightness > 0) {
		if (!led->pdata->led->power2v8on && (led->pdata->led->rgb_isnk_on)) {
			if (aw2023_2v8_power_on(led->pdata->led, true)) {
				dev_err(&led->pdata->led->client->dev, "2v8 power on failed");
			}
		}
	} else {
		if (led->pdata->led->power2v8on && !(led->pdata->led->rgb_isnk_on)) {
			if (aw2023_2v8_power_on(led->pdata->led, false)) {
				dev_err(&led->pdata->led->client->dev, "2v8 power off failed");
			}
		}
	}
	...
	if(led->id == AW2023_LED_BLUE) {
		...
		if((led_default[AW2023_LED_RED].pdata->led_mode == led_default[AW2023_LED_GREEN].pdata->led_mode)
			...
			aw2023_write(led, AW2023_REG_LCFG0, val & (~AW2023_LED_SYNC_MODE_MASK));
...
static int aw2023_check_chipid(struct aw2023_led *led)
{
	...
	/* aw2023 led reset */
	ret = aw2023_write(led, AW2023_REG_RESET, AW2023_RESET_MASK);
	if (ret) {
		dev_err(&led->client->dev, "reset aw2023 error = %d\n",ret);
	}
	msleep(5);
				...
				const struct i2c_device_id *id)
{
	...
	led_array->gpio_pwr_ctl = of_property_read_bool(node, "use_gpio_power_control");
	led_array->enable2v8_gpio = of_get_named_gpio(node, "enable2v8_gpio", 0);
	if (led_array->enable2v8_gpio < 0) {
		pr_err("aw2023[LED]:led->enable2v8_gpio not specified\n");
	} else {
		if (gpio_is_valid(led_array->enable2v8_gpio)) {
			ret = gpio_request(led_array->enable2v8_gpio, "vdd2v8-gpio");
			if (ret) {
				pr_err("aw2023[LED]:unable to request gpio [%d] %d\n", led_array->enable2v8_gpio, ret);
			}
		}
	}
	...
	led_array->rgb_isnk_on = 0;

	if(!led_array->poweron)
	{
		ret = aw2023_power_on(led_array, true);
		if(ret) {
			if(aw2023_power_on(led_array, true)) {
