...
enum AW2023_LED_MODE{
	...
	AW2023_LED_INDIVIDUAL_CTL_BREATH,
	AW2023_LED_NEW_ALWAYSON,
	AW2023_LED_MAXMODE,
};

enum AW2023_LED_TYPE {
	AW2023_LED_RED = 0,
	AW2023_LED_GREEN,
	AW2023_LED_BLUE,
	AW2023_LED_ON,
...
struct aw2023_led {
	...
	bool poweron;
	bool esd_flag;
	struct delayed_work   aw2023_led_work;
	struct workqueue_struct *aw2023_led_wq;
};

struct aw2023_led *led_default;
unsigned int aw2023_debug = 2;
...
static int aw2023_power_on(struct aw2023_led *led, bool on)
{
	...
	if (gpio_is_valid(led->vbled_enable_gpio)){
		if (on) {
			gpio_set_value_cansleep(led->vbled_enable_gpio, 1);
			msleep(AW_LED_POWER_OFF_DELAY);
			led->poweron = true;
		} else {
			gpio_set_value_cansleep(led->vbled_enable_gpio, 0);
			msleep(AW_LED_POWER_OFF_DELAY);
			led->poweron = false;
		}
		return 0;
...
static int aw2023_led_change_mode(struct aw2023_led *led, enum AW2023_LED_MODE mode)
{
	...
	switch(mode) {
		...
		case AW2023_LED_BREATHMODE:
			...
			led->pdata->led_mode = AW2023_LED_BREATH_MODE_MASK;
			break;
		case AW2023_LED_INDIVIDUAL_CTL_BREATH:
			led->pdata->hold_time_ms = 0;
			led->pdata->off_time_ms =  0;
			led->pdata->rise_time_ms = 6;
			led->pdata->fall_time_ms = 6;
			led->pdata->led_mode = AW2023_LED_INDIVIDUAL_CTL_BREATH;
			break;
		case AW2023_LED_NEW_ALWAYSON:
			led->pdata->led_mode = AW2023_LED_NEW_ALWAYSON;
			break;
		default:
			led->pdata->led_mode = AW2023_LED_MANUAL_MODE_MASK;
			break;
...
static void aw2023_brightness(struct aw2023_led *led)
{
	u8 i = 0;
	u8 val = 0;
	u8 enable = 0;
	u8 state = AW2023_LED_OFF;

	if (oplus_boot_mode_is_power_off_charging()) {
		AW2023_DEBUG("boot_mode is power_off_charging");
		return;
	}
	...
	if(led->pdata->led_mode == AW2023_LED_INDIVIDUAL_CTL_BREATH) {
		if(!led->cdev.brightness) {
			memset(rgb_color, 0, sizeof(rgb_color));
			aw2023_write(led, AW2023_REG_LEDEN, AW2023_LED_DISABLE_MASK);
		...
		for(i = 0; i < LED_MAX_NUM; i++) {
			aw2023_write(led, AW2023_REG_LED0T0 + i*LED_MAX_NUM,
				(led->pdata->rise_time_ms << AW2023_LED_TIME_SHIFT_MASK | led->pdata->hold_time_ms));
			aw2023_write(led, AW2023_REG_LED0T1 + i*LED_MAX_NUM,
				(led->pdata->fall_time_ms << AW2023_LED_TIME_SHIFT_MASK | led->pdata->off_time_ms));
			aw2023_write(led, AW2023_REG_PWM0 + i, AW2023_LED_PWM_MASK);
			aw2023_write(led, AW2023_REG_LCFG0 + i, AW2023_LED_BREATH_MODE_MASK | rgb_color[i]);
		}
		AW2023_DEBUG("AW2023_LED_INDIVIDUAL_CTL_BREATH:r=%d g=%d b=%d\n", rgb_color[0], rgb_color[1], rgb_color[2]);
		aw2023_read(led, AW2023_REG_LCFG0, &val);
		aw2023_write(led, AW2023_REG_LCFG0, AW2023_LED_SYNC_MODE_MASK | val);
		aw2023_write(led, AW2023_REG_LEDEN, AW2023_LED_ENABLE_MASK);
	}

	if(led->pdata->led_mode == AW2023_LED_NEW_ALWAYSON) {
		if(!led->cdev.brightness) {
			memset(rgb_color, 0, sizeof(rgb_color));
			aw2023_write(led, AW2023_REG_LEDEN, AW2023_LED_DISABLE_MASK);
			aw2023_write(led, AW2023_REG_GCR1, AW2023_CHIP_DISABLE_MASK);
		...
		for(i = 0; i < LED_MAX_NUM; i++) {
			aw2023_write(led, AW2023_REG_PWM0 + i, AW2023_LED_PWM_MASK);
			aw2023_write(led, AW2023_REG_LCFG0 + i, new_always_on_color[i]);
		}
		AW2023_DEBUG("AW2023_LED_NEW_ALWAYSON:r=%d g=%d b=%d\n", new_always_on_color[0], new_always_on_color[1], new_always_on_color[2]);
		aw2023_read(led, AW2023_REG_LCFG0, &val);
		aw2023_write(led, AW2023_REG_LCFG0, AW2023_LED_SYNC_MODE_MASK | val);
		aw2023_write(led, AW2023_REG_LEDEN, AW2023_LED_ENABLE_MASK);
	}
	...
	if((led->pdata->led_mode == AW2023_LED_BREATH_MODE_MASK) || (led->pdata->led_mode == AW2023_LED_MANUAL_MODE_MASK)) {
		/* aw2023 led breath time tr1 & ton */
		...
		if (led->cdev.brightness > 0) {
			...
			aw2023_write(led, AW2023_REG_PWM0 + led->id, AW2023_LED_PWM_MASK);
		}

		if((led->cdev.brightness == 0) && (led->id == AW2023_LED_RED || led->id == AW2023_LED_GREEN)){
			aw2023_write(led, AW2023_REG_LEDEN, AW2023_LED_DISABLE_MASK);
		}
		...
		if(led->id == AW2023_LED_BLUE) {
			aw2023_read(led, AW2023_REG_LCFG0, &val);
			if((led_default[AW2023_LED_RED].pdata->led_mode == led_default[AW2023_LED_GREEN].pdata->led_mode)
				&& (led_default[AW2023_LED_RED].pdata->led_mode == led_default[AW2023_LED_BLUE].pdata->led_mode)){
					aw2023_write(led, AW2023_REG_LCFG0, AW2023_LED_SYNC_MODE_MASK | val);
				}
			aw2023_write(led, AW2023_REG_LEDEN, AW2023_LED_ENABLE_MASK);
		}
	...
	if (val == 0 && led->id == AW2023_LED_BLUE) {
		aw2023_write(led, AW2023_REG_GCR1, AW2023_CHIP_DISABLE_MASK);
		if (aw2023_power_on(led->pdata->led, false)) {
...
static ssize_t aw2023_led_color_attr_show (struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct led_classdev *led_cdev = dev_get_drvdata(dev);
	struct aw2023_led *led = container_of(led_cdev, struct aw2023_led, cdev);
...
static ssize_t aw2023_led_color_attr_store(struct device *dev,
                struct device_attribute *attr, const char *buf, size_t cnt)
{
    	unsigned long data = 0;
    	struct led_classdev *led_cdev = dev_get_drvdata(dev);
...
void store_effect_color_and_brightness(char *tmp_buf)
{
	...
	if (str == NULL) {
		AW2023_DEBUG("error in parsing 1.1");
		return;
	}
	ret = kstrtoul(str, 10, &data);
	if (ret) {
		AW2023_DEBUG("error in parsing 1.2");
		return;
	}
	effect_index = data;
...
static ssize_t aw2023_effectdata_attr_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t cnt)
{
	unsigned long data = 0;
	char *delim = ",";
	char *str = NULL;
	char *p_buf = NULL, *tmp_buf = NULL;
	ssize_t ret = -EINVAL;

	AW2023_DEBUG("effectdata raw %s\n", buf);
	...
	struct device_attribute *attr, const char *buf, size_t cnt)
{
	...
	AW2023_DEBUG("%s: aw2023_debug = %u (max:15)\n", __func__, aw2023_debug);
...
static DEVICE_ATTR(color, 0664, aw2023_led_color_attr_show, aw2023_led_color_attr_store);
static DEVICE_ATTR(effectdata, 0664, NULL, aw2023_effectdata_attr_store);
...
static int aw2023_led_individual_ctl_breath_activate(struct led_classdev *cdev)
{
	int ret = 0;
	struct aw2023_led *led = container_of(cdev, struct aw2023_led, cdev);
	...
	ret = aw2023_led_change_mode(led, AW2023_LED_INDIVIDUAL_CTL_BREATH);
	if (ret < 0) {
		dev_err(led->cdev.dev, "%s: aw2023_led_change_mode fail\n", __func__);
		return ret;
	}
...
static void aw2023_led_individual_ctl_breath_deactivate(struct led_classdev *cdev)
{
	struct aw2023_led *led = container_of(cdev, struct aw2023_led, cdev);
...
static int aw2023_led_new_always_on_activate(struct led_classdev *cdev)
{
	int ret = 0;
	struct aw2023_led *led = container_of(cdev, struct aw2023_led, cdev);
	...
	ret = aw2023_led_change_mode(led, AW2023_LED_NEW_ALWAYSON);
	if (ret < 0) {
		dev_err(led->cdev.dev, "%s: aw2023_led_change_mode fail\n", __func__);
		return ret;
	}
...
static void aw2023_led_new_always_on_deactivate(struct led_classdev *cdev)
{
	struct aw2023_led *led = container_of(cdev, struct aw2023_led, cdev);
...
static struct led_trigger aw2023_led_trigger[LEDMODE_MAX_NUM] = {
	{
		.name = "cc_mode",
	...
	},
	{
		...
		.activate = aw2023_led_individual_ctl_breath_activate,
		.deactivate = aw2023_led_individual_ctl_breath_deactivate,
	},
	{
		.name = "new_always_on_mode",
		.activate = aw2023_led_new_always_on_activate,
		.deactivate = aw2023_led_new_always_on_deactivate,
	},
};
...
static int aw2023_check_chipid(struct aw2023_led *led)
{
	...
	for(cnt = 5; cnt > 0; cnt --)
	{
		...
		dev_err(&led->client->dev,"aw2023 chip id %0x", val);
...
static void aw2023_work_func(struct work_struct *aw2023_work)
{
	u8 ret = 0;
	u8 val = 0;
	struct aw2023_led *led = container_of(aw2023_work, struct aw2023_led, aw2023_led_work.work);

	ret = aw2023_read(led, AW2023_REG_STATUS, &val);
	if(val) {
		dev_notice(&led->client->dev, "%s AW2023_REG_STATUS[%d] : [0x%x]\n", __func__, led->id, val);
		led_default->esd_flag = true;
		ret = aw2023_init(led_default);
		if (ret) {
	...
	queue_delayed_work(led->aw2023_led_wq, &led->aw2023_led_work, 3 * HZ);
}
...
#endif
{
	...
	if (oplus_boot_mode_is_power_off_charging()) {
		AW2023_DEBUG("boot_mode is power_off_charging skip probe");
		return 0;
	}
	...
	if(!led_array->poweron)
	{
		...
		if(ret) {
			if(aw2023_power_on(led_array->pdata->led, true)) {
...
static int __init aw2023_led_init(void)
{
	...
	ret = i2c_add_driver(&aw2023_led_driver);
	if (ret) {
