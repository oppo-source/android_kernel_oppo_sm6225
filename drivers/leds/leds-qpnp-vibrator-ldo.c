...
static enum hrtimer_restart vib_stop_timer(struct hrtimer *timer)
{
	...
	chip->state = 0;
	queue_work(system_highpri_wq, &chip->vib_work);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(val < chip->min_duration_ms)
		val = chip->min_duration_ms;
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	ret = of_property_read_u32(of_node, "qcom,min-duration-ms",
				&chip->min_duration_ms);
	if (ret < 0) {
		chip->min_duration_ms = QPNP_VIB_MIN_PLAY_MS;
		pr_err("qcom,min-duration-ms property read failed, ret=%d\n",
			ret);
	}
	pr_debug("qcom,min-duration-ms =%d\n", chip->min_duration_ms);
