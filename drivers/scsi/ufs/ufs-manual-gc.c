...
void init_manual_gc(struct ufs_hba *hba)
{
	...
	snprintf(wq_name, MAX_WQ_NAME, "ufs_mgc_hibern8_work_%d",
			hba->host->host_no);
...
static struct attribute *mgc_sysfs_ufshcd_attrs[] = {
	&dev_attr_manual_gc.attr,
	&dev_attr_manual_gc_hold.attr,
	NULL
};

static const struct attribute_group mgc_sysfs_default_group = {
	.attrs = mgc_sysfs_ufshcd_attrs,
};

static void manual_gc_update_sysfs_work(struct work_struct *work)
{
	struct ufs_qcom_host *ufs = container_of(work, struct ufs_qcom_host,
						update_sysfs_work);
	...
	err = sysfs_update_group(&hba->dev->kobj,
				&mgc_sysfs_default_group);
	if (err)
		dev_err(hba->dev, "%s: Failed to add a manual gc group\n",
				__func__);
}

static void manual_gc_update_sysfs(void *data, struct ufs_hba *hba)
{
	struct ufs_qcom_host *ufs = ufshcd_get_variant(hba);
...
int init_manual_gc_sysfs(struct ufs_hba *hba)
{
	struct ufs_qcom_host *ufs = ufshcd_get_variant(hba);
	int ret;

	ret = register_trace_android_vh_ufs_update_sysfs(
				manual_gc_update_sysfs, NULL);
	if (ret)
		return ret;

	INIT_WORK(&ufs->update_sysfs_work, manual_gc_update_sysfs_work);
	return 0;
}
