...
/* wangchao@ODM.BSP.charge, 2019/12/6, Add for oplus_gauge*/
#include <linux/rtc.h>
#include "../../oplus/oplus_gauge.h"
/* Yuzhe.Peng@ODM.BSP.CHARGE, 2020/05/23, Add for battery proc devinfo */
...
#ifdef OPLUS_FEATURE_CHG_BASIC
/* zhangkun@BSP.CHG.Basic, 2020/04/22, Add for oplus_gauge*/
...
enum project_index {
	...
	RUM_PROJECT,
//#ifdef OPLUS_FEATURE_CHG_BASIC
/*linzhenming@BSP.CHG.BASIC, 20201210 , add for coco bat id*/
        COCO_PROJECT,
...
#ifdef OPLUS_FEATURE_CHG_BASIC
/* Yuzhe.Peng@ODM.BSP.CHARGE 2020/07/26 Add global ibat for battery temperture compensation */
int ibat_ma_adc_comp = 0;
#endif
...
#ifdef OPLUS_FEATURE_CHG_BASIC
/* zhangkun@BSP.CHG.Basic, 2020/04/22, Add for oplus_gauge*/
//qg-util.c start, wangchao@ODM.BSP.charge, 2019/12/6, Add for oplus_gauge
...
{
	if (chip->qg_subtype == QG_ADC_IBAT_5A && !chip->dt.qg_hi_current_cfg)
...
#ifdef OPLUS_FEATURE_CHG_BASIC
/* wangxianfei@ODM.BSP.charge, 2020/5/28, Add for Batt_NTC ADC compensation */
int g_oppo_qg_ibta;
...
static int oppo_asic_vadc_map_voltage_temp(const struct asic_vadc_map_pt *pts,
...
#endif
//#ifdef OPLUS_FEATURE_CHG_BASIC
/*linzhenming@BSP.CHG.BASIC, 20201210 , add for coco Batt_NTC ADC compensation*/
#ifdef OPLUS_FEATURE_CHG_BASIC
    if (chip->project_index == COCO_PROJECT){
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	/* wangxianfei@ODM.BSP.charge, 2020/5/28, Add for Batt_NTC ADC compensation */
	ibat_ma_adc_comp = g_oppo_qg_ibta = *ibat_ua / 1000;
	pr_err("qg ibat_ua=%d\n", *ibat_ua);
#endif
...
#endif
//qg-util.c end, wangchao@ODM.BSP.charge, 2019/12/6, Add for oplus_gauge
...
static void qg_notify_charger(struct qpnp_qg *chip)
{
	...
	if (!chip->profile_loaded)
		return;
#ifdef OPLUS_FEATURE_CHG_BASIC
	/* wangxianfei@ODM.BSP.charge, Fcv only can be setted by opluschg_set_fv,but no battery profile max-voltage-uv */
#else
...
static int qg_esr_estimate(struct qpnp_qg *chip)
{
	...
	if (chip->charge_status != POWER_SUPPLY_STATUS_CHARGING &&
		...
		return 0;
#ifdef OPLUS_FEATURE_CHG_BASIC
/* wangxianfei@BSP.CHG.Basic 2020/08/20,  CR#2730425 Add a logic to ignore ESR based on battery temperature.*/
		rc = qg_get_battery_temp(chip, &temp);
		if (rc < 0)
			return rc;
			       ...
			       union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_RESISTANCE_ID:
		/* Yuzhe.Peng@ODM.BSP.CHARGE 2020/05/26 Fix battery id accuracy loss */
		#ifdef OPLUS_FEATURE_CHG_BASIC
...
static int qg_parallel_status_update(struct qpnp_qg *chip)
{
	...
	/* do not sum SMB's current */
	if (chip->dt.qg_hi_current_cfg)
		return 0;
...
#ifdef OPLUS_FEATURE_CHG_BASIC
/* Yuzhe.Peng@ODM.BSP.CHARGE 2020/06/16 Add battery id check */
...
static struct batt_info soda_batt_info[3] =
{
...
{
	{150000, {820, 1100}, "ATL", "V1.0"}
};
//#ifdef OPLUS_FEATURE_CHG_BASIC
/*linzhenming@BSP.CHG.BASIC, 20201210 , add for coco bat id*/
static struct batt_info coco_batt_info[4] =
{
    {1000, {70, 180}, "Liwinon", "V1.0"},
    {15000, {180, 350}, "Coslight", "V1.0"},
    {68000, {550, 790}, "ATL", "V1.0"},
    {1000, {1040, 1325}, "SDI", "V1.0"}
};
//#endif /* OPLUS_FEATURE_CHG_BASIC */
...
static bool is_batt_id_valid(struct qpnp_qg *chip)
{
	...
	pr_info("id_batt_id_valid project number=%u, battery id voltage=%d\n",
		project_num, batt_id_voltage);

	if (chip->project_index == SODA_PROJECT) {
		proj_batterys.batt_info = soda_batt_info;
		proj_batterys.batt_vendor_count = sizeof(soda_batt_info) / sizeof(struct batt_info);
    ...
    //#ifdef OPLUS_FEATURE_CHG_BASIC
    /*linzhenming@BSP.CHG.BASIC, 20201210 , add for coco bat id*/
	} else if (chip->project_index == COCO_PROJECT) {
        proj_batterys.batt_info = coco_batt_info;
		proj_batterys.batt_vendor_count = sizeof(coco_batt_info) / sizeof(struct batt_info);
    //#endif /* OPLUS_FEATURE_CHG_BASIC */
    }else {
		pr_err("id_batt_id_valid unkonwn project type=%u\n", project_num);
		return true;
	}
...
static int qg_load_battery_profile(struct qpnp_qg *chip)
{
	...
	if (chip->dt.multi_profile_load) {
		...
		#ifdef OPLUS_FEATURE_CHG_BASIC
		/* Yuzhe.Peng@ODM.BSP.CHARGE 2020/05/22 Fix loss of accuracy when divide */
...
static int qg_setup_battery(struct qpnp_qg *chip)
{
	...
	if (!is_battery_present(chip)) {
		...
		if (rc < 0) {
			...
			/* Yuzhe.Peng@ODM.BSP.CHARGE 2020/05/26 Calculate battery id by rounding */
			#ifdef OPLUS_FEATURE_CHG_BASIC
...
static int qg_determine_pon_soc(struct qpnp_qg *chip)
{
	 ...
	 */
	pr_err("shutdown_soc_threshold:%d\n",chip->dt.shutdown_soc_threshold);
...
static int qg_hw_init(struct qpnp_qg *chip)
{
	...
	if (chip->dt.qg_ext_sense) {
		reg =0x83;
		rc = qg_write(chip, 0x305e,
					&reg, 1);
		if (rc < 0) {
			pr_err("Failed to Enable Ext Rsns with 10mOhms(0x305e) -> 0x83 rc=%d\n", rc);
			return rc;
		}
		reg =0x02;
		rc = qg_write(chip, 0xc140,
					&reg, 1);
		if (rc < 0) {
			pr_err("Failed to Enable Ext Rsns with 10mOhms(0x305e) -> 0x83 rc=%d\n", rc);
			return rc;
		...
		pr_err("Enable Ext Rsns with 10mOhms(0x305e) -> 0x83 0xc140=0x02\n");
	}
...
#ifdef OPLUS_FEATURE_CHG_BASIC
/* Yi.Zhou@ODM.BSP.CHARGE 2020/09/02 Add battery id check */
	rc = of_property_read_u32(node, "qcom,project-index", &temp);
	if (rc < 0)
		chip->project_index = INVALID_PROJECT;
	else
		chip->project_index = temp;
	pr_err("DT project index = %d\n",chip->project_index);
#endif
	/* IBAT thresholds */
	...
	chip->dt.qg_hi_current_cfg = of_property_read_bool(node, "qcom,qg-hi-current-config");
	...
	qg_dbg(chip, QG_DEBUG_PON, "DT: vbatt_empty_mv=%dmV vbatt_low_mv=%dmV delta_soc=%d ext-sns=%d \
			qg_hi_current_cfg=%d s3_entry_ibat_ua=%d s3_exit_ibat_ua=%d\n",
			chip->dt.vbatt_empty_mv, chip->dt.vbatt_low_mv, chip->dt.delta_soc, chip->dt.qg_ext_sense,
			chip->dt.qg_hi_current_cfg, chip->dt.s3_entry_ibat_ua, chip->dt.s3_exit_ibat_ua);
...
#ifdef OPLUS_FEATURE_CHG_BASIC
    /* Yi.Zhou  ODM@HQ.BSP.CHG.Basic  2020-8-10  add parallel current now */
