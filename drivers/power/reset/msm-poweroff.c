...
#ifdef VENDOR_EDIT
//Changan.Zhang@BSP.Kernel.Stability,add 2020/08/30 for recovery panic in aging version
...
//Changan.Zhang@BSP.Kernel.Stability,add 2020/08/30 for recovery panic in aging version
            if(get_eng_version() == AGING) {
                printk("[msm_restart_prepare] error:Prohibit aging version go in recovery mode!!\n");
                //BUG();
            }
#endif /*VENDOR_EDIT*/
