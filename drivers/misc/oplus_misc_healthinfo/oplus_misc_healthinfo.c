...
static int proc_misc_healthinfo_read(struct seq_file *s, void *v)
{
	...
	for (i = 0; i < MAX_LIGHTS; i++) {
		seq_printf(s, "name:light_%s,brightness:%llu,time(s):%llu\n",
		...
		OMH_DEBUG("name:light[%d],brightness:%llu,time(ms):%llu", i, lights_health_para[i][6], lights_health_para[i][5]);
	}
