 ...
 * 20200415 BSP PL added file and remove all added log.
 */
...
int hw_version_set()
{
	...
	unsigned int num = pcb % 8;
	if (id >= sizeof(pcb_versions)/sizeof(pcb_versions[0]))
		id = 0;

	switch (id) {
		case 0:  /* for unknow/PVT/MP */
		case 5:
		case 6:
			snprintf(hardware_id, sizeof(hardware_id), pcb_versions[id]);
			break;
		case 2:  /* just for T0 */
			if (num == 0) {
				snprintf(hardware_id, sizeof(hardware_id), pcb_versions[id]);
			...
			snprintf(hardware_id, sizeof(hardware_id), "%s-%u", pcb_versions[id], num);
			break;
		default:
			num += 1;
			snprintf(hardware_id, sizeof(hardware_id), "%s-%u", pcb_versions[id], num);
...
//int tid_hardware_info_get(char *, int);
...
static int __init board_id_setup(char *str)
{
	...
	//strncpy(board_id, str, strlen(str));
	strlcpy(board_id, str, sizeof(board_id));
	pr_err("board_id :%s\n", board_id);
	return 0;
}
//early_param("board_id", board_id_setup);
__setup("androidboot.prjname=", board_id_setup);
...
static int __init hardware_id_setup(char *str)
{
	...
	//strncpy(hardware_id, str, strlen(str));
	strlcpy(hardware_id, str, sizeof(hardware_id));
	pr_err("hardware_id :%s\n", hardware_id);
	return 0;
}
//early_param("hwversion", hardware_id_setup);
	...
	unsigned int cmd, unsigned long arg)
{
	...
	case HARDWARE_WIFI_GET:
		hardwareinfo_set_prop(HARDWARE_WIFI, "Qualcomm:wcn3988");
	...
	case HARDWARE_FM_GET:
		hardwareinfo_set_prop(HARDWARE_FM, "Qualcomm:wcn3991");
