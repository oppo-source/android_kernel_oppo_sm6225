...
#ifdef CONFIG_HYBRIDSWAP_CORE
static void meminfo_proc_show_hook(void *data, struct seq_file *m)
{
	...
	seq_printf(m, "EST:%20lu kB\n", eswap_total_pages << (PAGE_SHIFT - 10));
	seq_printf(m, "ESU_C:%18lu kB\n", eswap_compressed_pages << (PAGE_SHIFT - 10));
	seq_printf(m, "ESU_O:%18lu kB\n", eswap_used_pages << (PAGE_SHIFT - 10));
	seq_printf(m, "ZST:%20lu kB\n", zram_total_pags << (PAGE_SHIFT - 10));
	seq_printf(m, "ZSU_C:%18lu kB\n", zram_compressed >> 10);
	seq_printf(m, "ZSU_O:%18lu kB\n", zram_used_pages << (PAGE_SHIFT - 10));
}
#endif
...
static int register_all_hooks(void)
{
	...
	/* meminfo_proc_show_hook */
	REGISTER_HOOK(meminfo_proc_show);
#endif
	return 0;

#ifdef CONFIG_HYBRIDSWAP_CORE
	UNREGISTER_HOOK(meminfo_proc_show);
ERROR_OUT(meminfo_proc_show):
		...
		char *buf, size_t nbytes, loff_t off)
{
	...
	if (reclaim_total)
		nr_need_reclaim = lruvec_page_state(lruvec, NR_ACTIVE_ANON) +
			lruvec_page_state(lruvec, NR_INACTIVE_ANON);
	else
		nr_need_reclaim = lruvec_page_state(lruvec, NR_INACTIVE_ANON);
...
static int memcg_total_info_per_app_show(struct seq_file *m, void *v)
{
	...
	while ((memcg = get_next_memcg(memcg))) {
		...
		lruvec = mem_cgroup_lruvec(NODE_DATA(0), memcg);
		if (!lruvec) {
			get_next_memcg_break(memcg);
			return 0;
		}

		anon_size = lruvec_page_state(lruvec, NR_ACTIVE_ANON) +
			lruvec_page_state(lruvec, NR_INACTIVE_ANON);
