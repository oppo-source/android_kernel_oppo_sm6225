		...
		pg_data_t *pgdat)
{
	...
	if (!lruvec)
		return false;
	anon_total = lruvec_page_state(lruvec, NR_ACTIVE_ANON) +
		lruvec_page_state(lruvec, NR_INACTIVE_ANON) +
...
static inline u64 calc_shrink_ratio(pg_data_t *pgdat)
{
	...
	while ((memcg = get_next_memcg(memcg))) {
		...
		nr_anon = lruvec_page_state(lruvec, NR_ACTIVE_ANON) +
			lruvec_page_state(lruvec, NR_INACTIVE_ANON);
		...
		unsigned long nr_to_reclaim)
{
	...
	while ((memcg = get_next_memcg(memcg))) {
		...
		memcg_to_reclaim = hybs->can_reclaimed * nr_to_reclaim / total_can_reclaimed;
		memcg_nr_reclaimed = try_to_free_mem_cgroup_pages(memcg,
				memcg_to_reclaim, GFP_KERNEL, true);
		reclaim_memcg_cnt++;
		hybp(HYB_INFO, "memcg %s reclaim %lu\n", hybs->name,
				page_to_kb(memcg_nr_reclaimed));
