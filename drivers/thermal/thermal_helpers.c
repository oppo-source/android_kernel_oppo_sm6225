	...
	if (is_ntcswitch_projects()) {
		if (strcmp(tz->type, "cpu-therm-usr") == 0) {
			...
			*temp = oplus_thermal_tmp_get_chg();
			ret = 0;
		} else if (strcmp(tz->type, "pa-therm-usr") == 0) {
			*temp = oplus_thermal_tmp_get_pa();
			ret = 0;
		} else if (strcmp(tz->type, "batt0-therm-user") == 0) {
			*temp = oplus_thermal_tmp_get_batt();
			ret = 0;
		} else if (strcmp(tz->type, "vbus-btb-therm-usr") == 0) {
			*temp = oplus_thermal_tmp_get_vbus_btb();
			ret = 0;
		} else if (strcmp(tz->type, "batt-btb-therm-usr") == 0) {
			*temp = oplus_thermal_tmp_get_batt_btb();
			ret = 0;
