...
#ifdef CONFIG_OPLUS_CHARGER_BQ25890H
#include "../../../power/oplus/charger_ic/oplus_bq25890h_charger.h"
...
#define USB_TYPE_POLLING_CNT_MAX	150
#define USB_TYPE_POLLING_CNT            10
extern void notify_pd_event(unsigned long evt);
enum dr {
	DR_IDLE,
	DR_DEVICE,
	DR_HOST,
...
static void usb_dwork_handler(struct work_struct *work)
{
	...
	case DR_DEVICE:
		...
		if (ret < 0 || val.intval == POWER_SUPPLY_TYPE_UNKNOWN) {
			...
			break;
		} else if (ret < 0 || val.intval == POWER_SUPPLY_TYPE_USB_DCP) {
			if (rpmd->usb_type_polling_cnt < USB_TYPE_POLLING_CNT)
				schedule_delayed_work(&rpmd->usb_dwork,
						msecs_to_jiffies(
						USB_TYPE_POLLING_INTERVAL));
			break;
				...
				unsigned long event, void *data)
{
	...
	case TCP_NOTIFY_PD_STATE:
		...
		case PD_CONNECT_NONE:
			notify_pd_event(PD_CONNECT_NONE);
			break;
		case PD_CONNECT_HARD_RESET:
			notify_pd_event(PD_CONNECT_HARD_RESET);
			break;
		case PD_CONNECT_PE_READY_SNK:
			notify_pd_event(PD_CONNECT_PE_READY_SNK);
			break;
		case PD_CONNECT_PE_READY_SNK_PD30:
			notify_pd_event(PD_CONNECT_PE_READY_SNK_PD30);
			break;
		case PD_CONNECT_PE_READY_SNK_APDO:
			notify_pd_event(PD_CONNECT_PE_READY_SNK_APDO);
			break;
