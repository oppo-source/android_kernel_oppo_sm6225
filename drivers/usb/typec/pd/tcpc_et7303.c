...
#ifdef OPLUS_FEATURE_CHG_BASIC
	if(!(chip->disable_irq_reset && chip->chip_id == RT1715_DID_D))
		rt1711_i2c_write8(tcpc,
...
static inline int rt1711h_check_revision(struct i2c_client *client)
{
	...
	if ((vid != RICHTEK_1711_VID) && (vid != ETEK_7303_VID) && (vid != HUSB311_VID)) {
...
#else
int et7303_driver_init(void)
{
	int rc;

	pr_info("et7303_driver_init >>>");
	rc = i2c_add_driver(&rt1711_driver);
	if (rc < 0)
		pr_err("[OPLUS_CHG]: register et7303 driver failed, rc=%d", rc);

	return rc;
}
EXPORT_SYMBOL(et7303_driver_init);
void et7303_driver_exit(void)
{
	i2c_del_driver(&rt1711_driver);
}
EXPORT_SYMBOL(et7303_driver_exit);
#endif
