...
static int _aw37004_regulator_get_voltage(struct regulator_dev *rdev)
{
	...
	rc = aw37004_read(fan_reg, AW37004_REG_ID, &val, 1);
	if (rc < 0) {
		LDO_LOGE(fan_reg, "aw37004 failed to get PID rc = %d", rc);
		return rc;
	}
	else{
		LDO_LOGI(fan_reg, "aw37004 get Product ID: [%02x]\n", val);
	}
	...
	if (vset == 0) {
		...
		if (val == SGM38121_REGULATOR_CHIP_ID)
		{
				 ...
				 const struct i2c_device_id *id)
{
	...
	if(g_regmap != NULL){
		rc = regmap_read(g_regmap, AW37004_REG_ID, &val);
		if (rc < 0) {
	...
	/*second supplier enable OUT_DIS*/
	if(val == REGULATOR_CHIP_ID)
	{
		rc = regmap_write(g_regmap, AW37004_REG_RDIS, REGULATOR_LDO_RDIS);
		if (rc < 0) {
			LOGE("aw37004 0X02 failed rc = %d",rc);
		}
	}
