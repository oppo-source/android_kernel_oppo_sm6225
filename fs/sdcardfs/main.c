	...
	*debug = 0;
	...
	while ((p = strsep(&options, ",")) != NULL) {
		...
		case Opt_mask:
			if (match_int(&args[0], &option))
				return 0;
			opts->vfsopts.mask = 0007; /* 0007: full permission mask */
			break;
		case Opt_multiuser:
			opts->multiuser = true;
			break;
				...
				struct sdcardfs_vfsmount_options *vfsopts)
{
	...
	while ((p = strsep(&options, ",")) != NULL) {
		...
		case Opt_gid:
			if (match_int(&args[0], &option))
				return 0;
			vfsopts->gid = AID_MEDIA_RW;
			break;
		case Opt_mask:
			if (match_int(&args[0], &option))
				return 0;
			vfsopts->mask = 0007; /* 0007: full permission mask */
			break;
		case Opt_unshared_obb:
		case Opt_default_normal:
		case Opt_multiuser:
		...
		const char *dev_name, void *raw_data, int silent)
{
	...
	if (sb_info->options.multiuser) {
		setup_derived_state(d_inode(sb->s_root), PERM_PRE_ROOT,
				sb_info->options.fs_user_id, AID_MEDIA_RW);
