...
static inline int get_gid(struct sdcardfs_vfsmount_options *vfsopts,
		struct super_block *sb,
		struct sdcardfs_inode_data *data)
{
	...
	else if (data->under_data)
		return AID_EXT_DATA_RW;
	else if (data->under_obb)
		return AID_EXT_OBB_RW;
	else
		return multiuser_get_app_id(vfsopts->gid);
}

static inline int get_mode(struct sdcardfs_vfsmount_options *opts,
		struct sdcardfs_inode_info *info,
		struct sdcardfs_inode_data *data)
{
	int owner_mode;
	...
	if (data->perm == PERM_PRE_ROOT) {
		...
		*/
		visible_mode = 0751;
	} else if (data->perm == PERM_ANDROID_DATA || data->perm == PERM_ANDROID_OBB) {
		/* allow "other" access to data/obb dirs */
		visible_mode = 0771;
...
static inline int prepare_dir(const char *path_s, uid_t uid, gid_t gid, mode_t mode)
{
	...
	err = vfs_mkdir(&init_user_ns,d_inode(parent.dentry), dent, mode);
	if (err) {
		if (err == -EEXIST)
			err = 0;
		goto out_dput;
	...
	notify_change(&init_user_ns,dent, &attrs, NULL);
